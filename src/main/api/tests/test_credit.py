...
class TestCredit:
    def test_credit_req(self, db_session: Session, api_manager: ApiManager, create_credit_user_request: CreateUserRequest, create_credit_account_request, credit_data_request: CreditRequest):
        ...
        assert response.amount == response.balance == credit_data_request.amount, (
            f"Сумма на балансе {response.balance} не равна сумме зачисления {credit_data_request.amount}"
        )
        assert response.id == credit_data_request.accountId, (
            f"Id аккаунта счета не совпадает со счетом зачисления {credit_data_request.accountId}"
        )
        assert response.termMonths == credit_data_request.termMonths, (
            f"Срок кредита {response.termMoths} не соответствует заявке {credit_data_request.termMoths}"
        )
        assert credit_from_db.account_id == credit_data_request.accountId, "id запрашиваемого аккаунта не создан"
        assert credit_from_db.amount == credit_data_request.amount, "Сумма кредита != запрашиваемой сумме"
        assert credit_from_db.term_months == credit_data_request.termMonths, "Срок кредита не соответствует запрашиваемому"
    ...
    def test_second_credit_req(self, db_session: Session, api_manager: ApiManager, create_credit_user_request: CreateUserRequest, create_account_for_second_credit):
        ...
        assert response.json()["error"] == ErrorMessages.ONLY_ONE_CREDIT_BY_ONE_ACCOUNT, (
            f"Ожидалась ошибка {ErrorMessages.ONLY_ONE_CREDIT_BY_ONE_ACCOUNT}, а была получена {response.json()['error']}"
        )
    ...
    def test_credit_req_for_role_user(self, db_session: Session, api_manager: ApiManager, create_user_request: CreateUserRequest, credit_data_for_dont_credit_role_request: CreditRequest):
        ...
        assert (response.json()["title"] == ErrorMessages.FORBIDDEN_FOR_CREDIT and
                response.json()["detail"] == ErrorMessages.DETAIL_FORBIDDEN_FOR_CREDIT), (
