...
class TestTransfer:
    def test_transfer_to_yourself(self, db_session: Session, api_manager: ApiManager, create_user_request: CreateUserRequest, create_account_with_money_request: AddDepositRequest, transfer_data_request: TransferRequest):
        ...
        transaction_from_db = Transaction.get_transaction_by_same_id(db_session, to_account_id = transfer_data_request.toAccountId, from_account_id = transfer_data_request.fromAccountId)
        ...
        assert transaction_from_db.transaction_type == TestData.TRANSACTION_TYPE_TRANSFER, "Такой операции не найдено в БД"
        assert account_to.balance == transfer_data_request.amount, f"Сумма на счете не соответствует {transfer_data_request.amount}"
    ...
    def test_transfer_to_yourself_invalid_summ(self, db_session: Session, api_manager: ApiManager, create_user_request: CreateUserRequest, create_account_request, create_account_with_money_request: AddDepositRequest, transfer_data_request: TransferRequest, transfer: float):
        ...
        assert response.json()["error"] == ErrorMessages.INVALID_TRANSFER_AMOUNT, (
            f"Ожидалась ошибка {ErrorMessages.INVALID_TRANSFER_AMOUNT}, а была получена {response.json()["error"]}"
        )
        assert transaction_from_db is None, "Операция о переводе создана в БД"
        assert account_to.balance == TestData.START_DEPOSIT_AMOUNT, "Сумма на счете была пополнена и не равна 0"
        assert account_from.balance == create_account_with_money_request.balance, f"Со счета были списаны деньги"
