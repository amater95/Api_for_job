...
class TestAddDeposit:
    def test_add_deposit(self, db_session: Session, api_manager: ApiManager, create_user_request: CreateUserRequest, create_account_request):
        ...
        assert response.balance == add_deposit_request.amount, (
            f"После пополнения баланс должен быть равен {response.balance}, но он остался равен {add_deposit_request.amount}")
        assert response.id == add_deposit_request.accountId, (
            f"Неправильный счет пополнения"
        )
    ...
    def test_add_deposit_with_invalid_summ(self, db_session: Session, api_manager: ApiManager, create_user_request: CreateUserRequest, create_account_request, deposit: float):
        ...
        assert request.json()["error"] == ErrorMessages.INVALID_DEPOSIT_AMOUNT, (
            f"Ожидали ошибку {ErrorMessages.INVALID_DEPOSIT_AMOUNT}, а была получена {request.json()['error']}"
        )
