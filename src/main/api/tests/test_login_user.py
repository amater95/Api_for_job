...
class TestLoginUser:
    def test_login_admin(self, api_manager: ApiManager):
        ...
        assert login_admin_request.username == response.user.username, (
            f"Администратор имеет login = {login_admin_request.username}, а не = {response.user.username}"
        )
        assert response.user.role == TestData.ADMIN_ROLE, (
            f"Админ может иметь только роль = {TestData.ADMIN_ROLE}, но не = {response.user.role}"
        )
    ...
    def test_login_user(self, api_manager: ApiManager, create_user_request: CreateUserRequest):
        ...
        assert create_user_request.username == response.user.username, (
            f"Пользователь должен иметь имя {create_user_request.username}, а не = {response.user.username}"
        )
        assert response.user.role == TestData.USER_ROLE, (
            f"Пользователь имеет роль {TestData.USER_ROLE}, вместо = {response.user.role}"
        )
