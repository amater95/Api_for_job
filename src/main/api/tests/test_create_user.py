...
class TestCreateUser:
    ...
    def test_create_user(self, db_session: Session, api_manager: ApiManager, create_user_request: CreateUserRequest):
        ...
        assert create_user_request.username == response.username, (
            f"Пользователю должно было присвоиться имя {create_user_request.username} вместо {response.username} "
        )
        assert create_user_request.role == response.role, (
            f"Пользователю присвоилась роль {response.role} вместо передаваемой {create_user_request.role}"
        )
        assert user_from_db.username == create_user_request.username, (
            f"Пользователь {create_user_request.username} не был создан в БД"
        )
