...
class TestCreateAccount:
    def test_create_account(self, db_session: Session, api_manager: ApiManager, create_user_request: CreateUserRequest):
        ...
        assert response.balance == TestData.START_DEPOSIT_AMOUNT, (
            f"При создании нового аккаунта его баланс должен быть = {TestData.START_DEPOSIT_AMOUNT}, а получили {response.balance}"
        )
        assert account_from_db.id == response.id, "Аккаунт не добавлен в БД"
        assert account_from_db.balance is not None, "Поле с балансом не существует в БД"
    ...
    def test_create_three_accounts(self, db_session: Session, api_manager: ApiManager, create_user_request: CreateUserRequest):
        ...
        assert (
            response_third_account.json()["error"]
            == ErrorMessages.MAX_ACCOUNTS_LIMIT_REACHED
        ), (
            f"Ожидали получить ошибку о достижении максимального количества аккаунтов {ErrorMessages.MAX_ACCOUNTS_LIMIT_REACHED}"
        )
